...
def preferences_generation(
    states: list, preferred_states=[], dispreferred_states=[], players=2
):
    ...
    for state in sstates:
        if state in preferred_states:
            preferences[str(state)] = {"state": state, "score": 2}
        elif state in dispreferred_states:
            preferences[str(state)] = {"state": state, "score": 0}
        else:
            preferences[str(state)] = {"state": state, "score": 1}
    return preferences
