...
# Import packages
import pandas as pd
from players import HonestNdivSpeaker, Player
import seaborn as sns
import matplotlib.pyplot as plt


def viz(outcomes, type="speaker", full_results=True):
    cases = {"speaker": speaker_viz, "listener": listener_viz}
...
def speaker_viz(spk_outcomes):
    ...
    dfs = {
        key: {
            k: pd.DataFrame(
                {
                    "x": list(spk_outcomes[key][1][k].keys()),
    ...
    for key in df_pairs:
        ...
        sns.barplot(x="y", y="x", data=res, hue="hue", alpha=probs_alpha[i], ax=axs[i])
        axs[i].legend(
...
def listener_viz(lis_outcomes):
    for outcomes in lis_outcomes:
        ...
        for key in df_pairs:
            ...
            res = pd.concat(df_pairs[key])
            sns.barplot(
                x="y", y="x", data=res, hue="hue", alpha=probs_alpha[i], ax=axs[i]
            )
            axs[i].legend(
                bbox_to_anchor=(1.02, 1),
                loc="upper left",
...
def rsa_lis_viz(l0 : Player, 
socs : list, 
lexs : list, 
interpretation = "world_interpretation"):
    l0_preds = {k:v[interpretation] 
    for (k,v) in l0.full_predictions(socs, lexs).items()}
    ...
    ax.set(xlabel='Message', ylabel='P(w|m)',
        title='RSA predictions for the listener')
    ax.set_ylim([0, 1])
    plt.show()

def rsa_speak_viz(s1 : HonestNdivSpeaker, 
socs : list, 
lexs : list):
    ...
    ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.0),
          ncol=3, fancybox=True, shadow=True)
    ax.set(xlabel='Message', ylabel='P(m|w)',
        title='RSA predictions for the speaker')
    ax.set_ylim([0, 1])
    plt.show()
